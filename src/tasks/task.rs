use super::task_delete::TaskDeleteBucket;
...
use crate::{
    ...
    tasks::{task_errors::TaskError, LogInfo, Status},
};
...
impl Task {
    pub fn task_type(&self) -> TaskType {
        match self {
            ...
            Task::DeleteBucket(_) => TaskType::DeleteBucket,
        }
    }
    ...
    pub fn meta_dir(&self) -> String {
        match self {
            ...
            Task::DeleteBucket(d) => d.attributes.meta_dir.clone(),
        }
    }
    ...
    pub fn task_source(&self) -> ObjectStorage {
        match self {
            ...
            Task::Compare(c) => c.source.clone(),
            Task::DeleteBucket(d) => d.source.clone(),
        }
    }
    ...
    pub fn set_meta_dir(&mut self, meta_dir: &str) {
        match self {
            ...
            Task::DeleteBucket(d) => d.attributes.meta_dir = meta_dir.to_string(),
        }
    }
    ...
    pub fn set_task_id(&mut self, task_id: &str) {
        match self {
            ...
            Task::DeleteBucket(d) => d.task_id = task_id.to_string(),
        }
    }
    ...
    pub fn set_from_checkpoint(&mut self, from_checkpoint: bool) {
        match self {
            ...
            Task::DeleteBucket(d) => d.attributes.start_from_checkpoint = from_checkpoint,
        }
    }
    ...
    pub fn task_id(&self) -> String {
        return match self {
            ...
            Task::DeleteBucket(d) => d.task_id.clone(),
        };
    }
    ...
    // 删除meta目录
    pub fn clean_persistenced_status(&self) -> Result<()> {
        let task_id = self.task_id();
        if task_is_living(&task_id) {
            return Err(anyhow!("task is living"));
        }
        ...
        // 清理meta dir
        let _ = remove_dir(self.meta_dir());
        Ok(())
    }
    ...
    pub async fn execute(&self) {
        let r = match self {
            Task::Transfer(t) => t.start_transfer().await,
            Task::Compare(c) => c.start_compare().await,
            Task::DeleteBucket(d) => d.start_delete().await,
        };

        self.clean_runtime_status();

        let mut task_status = match get_task_status(&self.task_id()) {
            Ok(s) => s,
            Err(e) => {
                let err = TaskError {
